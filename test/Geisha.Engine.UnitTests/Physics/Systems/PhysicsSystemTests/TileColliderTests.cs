...
public class TileColliderTests : PhysicsSystemTestsBase
{
    ...
    public void TileBody_ShouldNotFilterCollisionNormal_WhenTileCollisionNormalFilterIsNone_Or_ItIsNotNoneButFilteredCollisionNormalIsZeroVector()
    {
        ...
        // Assert
        var tileWithNoneFilterContacts = new List<Contact2D>();
        tileWithNoneFilter.GetComponent<TileColliderComponent>().GetContacts(tileWithNoneFilterContacts);
        Assert.That(tileWithNoneFilterContacts[0].CollisionNormal, Is.EqualTo(-Vector2.UnitX));

        var tileWithZeroVectorContacts = new List<Contact2D>();
        tileWithZeroVector.GetComponent<TileColliderComponent>().GetContacts(tileWithZeroVectorContacts);
        Assert.That(tileWithZeroVectorContacts[0].CollisionNormal, Is.EqualTo(Vector2.UnitX));
    }
    ...
    public void TileBody_ShouldGenerateRegularContactPoints_ButShouldAdjustCollisionNormal()
    {
        ...
        kinematicBody.GetComponent<RectangleColliderComponent>().GetContacts(contacts);
